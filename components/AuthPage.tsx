...
const AuthPage: React.FC = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    // PARTNER MOBILE AUTH LOGIC (BYPASS EMAIL CHECKS)
    if (userType === 'Partner') {
      ...
      // Use mobile as identifier internally but hide it from the user
      const internalID = `${mobile}@partner.bb`;
      try {
        if (isLogin) {
          await signIn(internalID, password);
        } else {
          // Attempt signup, if "email-already-in-use" (mobile exists), silently switch to login
          try {
        ...
        setError('Network entry failed. Check your mobile number and password.');
      } finally {
        setIsSubmitting(false);
      }
    } else {
      ...
      try {
        if (isLogin) await signIn(mockEmail, password);
        else await signUp(mockEmail, password, { name: name || 'User' }, 'customer');
      } catch (err: any) {
    ...
    <div className="min-h-screen bg-white pt-32 pb-20 px-6 flex justify-center items-start">
      <div className="w-full max-w-md bg-white border border-gray-100 p-8 md:p-12 rounded-[2.5rem] shadow-sm">
        <div className="text-center mb-10">
          <h1 className="text-3xl font-serif font-bold text-charcoal mb-2 uppercase tracking-tight tracking-[-0.03em]">BB Connect Portal</h1>
          <p className="text-[10px] text-bbBlue font-bold uppercase tracking-[0.4em]">
            {userType === 'Partner' ? 'Professional Registry' : 'Secure Access Point'}
          </p>
        </div>
        ...
        <div className="relative mb-8 text-center">
          ...
          <span className="relative px-4 bg-white text-[9px] font-bold text-gray-300 uppercase tracking-[0.3em]">Network Access</span>
        </div>

        <form onSubmit={handleAuth} className="space-y-6">
          <div className="flex bg-gray-50 p-1 rounded-2xl border border-gray-100">
            <button type="button" onClick={() => {setUserType('Customer'); setIsLogin(false);}} className={`flex-1 py-3.5 text-[9px] font-bold uppercase tracking-widest rounded-xl transition-all ${userType === 'Customer' ? 'bg-white text-bbBlue shadow-sm' : 'text-gray-400 hover:text-charcoal'}`}>Customer</button>
            <button type="button" onClick={() => {setUserType('Partner'); setIsLogin(false);}} className={`flex-1 py-3.5 text-[9px] font-bold uppercase tracking-widest rounded-xl transition-all ${userType === 'Partner' ? 'bg-white text-bbBlue shadow-sm' : 'text-gray-400 hover:text-charcoal'}`}>Partner</button>
          </div>
          ...
          <button type="submit" disabled={isSubmitting} className="w-full py-5 bg-bbBlue text-white rounded-2xl font-bold uppercase text-xs tracking-[0.2em] shadow-xl shadow-bbBlue/20 hover:bg-bbBlue-deep transition-all active:scale-[0.98] disabled:opacity-50">
            {isSubmitting ? 'Verifying...' : isLogin ? 'Access Dashboard' : 'Initiate Registration'}
          </button>
        </form>
