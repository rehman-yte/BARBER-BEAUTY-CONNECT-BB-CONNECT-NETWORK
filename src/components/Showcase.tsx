    ...
    <section className="py-[6rem] bg-white">
      <div className="max-w-[1440px] mx-auto px-[5%]">
        <div className="text-center mb-[4rem]">
          <h2 className="text-[0.625rem] font-bold text-gray-400 uppercase tracking-[0.5em] mb-[1rem]">The Showcase</h2>
          <h3 className="text-[1.875rem] md:text-[2.5rem] font-serif font-bold text-charcoal">Visual Excellence</h3>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-[1rem] md:gap-[2rem]">
          {images.map((img, idx) => (
            <motion.div 
              key={idx}
              whileHover={{ y: -10 }}
              className="relative aspect-square rounded-[1.5rem] overflow-hidden shadow-lg group bg-gray-50"
            >
              ...
              <div className="absolute inset-0 bg-bbBlue-deep/40 transition-all duration-500 flex items-center justify-center opacity-0 group-hover:opacity-100 backdrop-blur-[2px]">
                <span className="text-white text-[0.75rem] font-bold uppercase tracking-[0.3em] border-b border-white/50 pb-[0.25rem]">{img.label}</span>
              </div>
            </motion.div>
          ))}
        </div>
