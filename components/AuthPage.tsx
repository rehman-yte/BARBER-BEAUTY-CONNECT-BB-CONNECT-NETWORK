...
const AuthPage: React.FC = () => {
  ...
  // EMERGENCY REDIRECT: Use localStorage as a secondary source of truth to break loops
  useEffect(() => {
    const isPartnerAuth = localStorage.getItem('bb_partner_auth');
    if (user && !loading) {
      if (user.role === 'customer') {
        ...
        if (user.status === 'active') {
          ...
          // Hard redirect to registration to bypass loop
          window.location.assign('#/partner-register');
        }
      }
    } else if (isPartnerAuth === 'true' && !user && !loading) {
      // If we flagged it but user isn't in context yet, wait or force refresh
      console.log("Partner authenticated, awaiting session...");
    }
  }, [user, loading, navigate]);

  const handleGoogleAuth = async () => {
    if (userType === 'Partner') return; // Strict block
    try {
      if (!auth || !db) return;
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (userType === 'Partner') {
      // 1. Mandatory testing OTP check
      if (!isLogin && otp !== '123456') {
        setError('Invalid OTP. Use 123456.');
        setIsSubmitting(false);
        return;
      }

      // 2. Silent Auth Protocol: Bypass "Email already in use" by trying sign-in first
      const internalID = `p_${mobile}@bb.net`;
      try {
        if (isLogin) {
          await signIn(internalID, password);
        } else {
          try {
            await signUp(internalID, password, { name: name || 'Partner' }, 'partner');
          } catch (signUpErr: any) {
            // If already exists, just sign in (silent upgrade/re-entry)
            await signIn(internalID, password);
          }
        }
        
        // 3. SECURE REDIRECT: Flag as authenticated and jump to the form
        localStorage.setItem('bb_partner_auth', 'true');
        window.location.assign('#/partner-register');

      } catch (err: any) {
        setError('Network verification failed. Please check credentials.');
        setIsSubmitting(false);
      }
    } else {
      // Customer Path
  ...
  return (
    <div className="min-h-screen bg-white pt-32 pb-20 px-6 flex justify-center items-start">
      <div className="w-full max-w-md bg-white border border-gray-100 p-8 md:p-12 rounded-[2.5rem] shadow-sm">
        ...
        {userType === 'Customer' && (
          <button onClick={handleGoogleAuth} className="w-full flex items-center justify-center gap-3 py-4 border border-gray-100 rounded-2xl hover:bg-gray-50 transition-all mb-8 group active:scale-[0.98]">
            ...
            <span className="text-xs font-bold text-charcoal tracking-widest uppercase">Sign In with Google</span>
          </button>
        )}
        ...
        <form onSubmit={handleAuth} className="space-y-6">
          ...
          <div className="space-y-4">
            {!isLogin && (
              <div className="flex flex-col gap-2">
                ...
                <input required type="text" placeholder="Professional Name" className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-sm outline-none focus:border-bbBlue transition-all" value={name} onChange={(e) => setName(e.target.value)} />
              </div>
            )}
            ...
            {userType === 'Partner' && !isLogin && isOtpSent && (
              <div className="flex flex-col gap-2">
                <label className="text-[9px] font-bold text-charcoal uppercase tracking-[0.2em] ml-1">OTP Gate (Demo: 123456)</label>
          ...
          <button type="submit" disabled={isSubmitting} className="w-full py-5 bg-bbBlue text-white rounded-2xl font-bold uppercase text-xs tracking-[0.2em] shadow-xl shadow-bbBlue/20 hover:bg-bbBlue-deep transition-all active:scale-[0.98] disabled:opacity-50">
            {isSubmitting ? 'Verifying...' : isLogin ? 'Access Registry' : 'Initiate Signup'}
          </button>
        </form>
        ...
        <div className="mt-8 text-center">
          <button onClick={() => setIsLogin(!isLogin)} className="text-[9px] font-bold text-gray-400 uppercase tracking-widest hover:text-bbBlue transition-colors">
            {isLogin ? "Join the Elite Network" : "Existing Partner? Access Portal"}
          </button>
        </div>
      </div>
    </div>
