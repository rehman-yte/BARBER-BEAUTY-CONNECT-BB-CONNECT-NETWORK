...
const PartnerRegistration: React.FC = () => {
  ...
  const handleMockUpload = (field: string, index?: number) => {
    ...
    if (typeof index === 'number') {
      ...
      newPics[index] = 'VERIFIED_ASSET_TOKEN';
      setFormData(prev => ({ ...prev, workerPics: newPics }));
    } else {
      setFormData(prev => ({ ...prev, [field]: 'VERIFIED_ASSET_TOKEN' }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Global Security Registry Scan (3s)
    setTimeout(async () => {
      if (user && db) {
        try {
    ...
    <div className="pt-32 pb-20 px-6 md:px-12 bg-white min-h-screen flex justify-center items-start">
      <AnimatePresence mode="wait">
        {!isProcessing ? (
          ...
          >
            <div className="mb-16">
              <h1 className="text-4xl md:text-5xl font-serif font-bold text-charcoal mb-4 uppercase tracking-tight tracking-[-0.04em]">Professional Registry</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-20">
              ...
              <section className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-100 rounded-3xl p-6 hover:border-bbBlue/30 transition-all cursor-pointer bg-gray-50/50" onClick={() => handleMockUpload('ownerPic')}>
                    ...
                    <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest leading-none">{formData.ownerPic ? 'Portrait Captured' : 'Upload Owner Portrait'}</span>
                  </div>
                </div>
              </section>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {formData.shopImages.map((img, i) => (
                        <input key={i} required placeholder={`Image ${i+1} URL`} value={img} onChange={(e) => handleShopImageChange(i, e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-[10px] focus:border-bbBlue outline-none transition-all" />
                      ))}
                    </div>
                  </div>
                </div>
              ...
              <section className="space-y-10">
                ...
                <div className="space-y-10">
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-6 gap-4 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
              ...
              {/* 04. SETTLEMENT & COMPLIANCE */}
              <section className="space-y-10">
                <h3 className="text-xs font-bold text-gray-300 uppercase tracking-[0.3em] border-b border-gray-50 pb-4">04. Legal & Settlement</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-8">
                    <div className="flex flex-col gap-4">
